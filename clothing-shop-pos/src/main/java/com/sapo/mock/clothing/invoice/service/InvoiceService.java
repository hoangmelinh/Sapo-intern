...
public class InvoiceService {
        ...
        for (ReqCreateInvoiceDTO.InvoiceItemDTO itemDto : dto.getItems()) {
            ProductVariant variant = productVariantRepository.findById(itemDto.getVariantId())
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Biến thể sản phẩm ID " + itemDto.getVariantId() + " không tồn tại"));
    ...
    public ResInvoiceDTO cancelInvoice(Integer id) {
        ...
        // Restore stock — dùng variantId từ snapshot để tra kho
        List<InvoiceItem> items = invoiceItemRepository.findByInvoiceId(id);
        for (InvoiceItem item : items) {
            WarehouseStock stock = warehouseStockRepository
                    .findByVariantIdAndWarehouseId(item.getVariantId(), invoice.getWarehouseId())
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Không tìm thấy thông tin tồn kho của biến thể ID " + item.getVariantId() + " trong kho "
    ...
    private void deductWarehouseStock(List<ReqCreateInvoiceDTO.InvoiceItemDTO> items, Integer warehouseId) {
        for (ReqCreateInvoiceDTO.InvoiceItemDTO itemDto : items) {
            WarehouseStock stock = warehouseStockRepository
                    .findByVariantIdAndWarehouseId(itemDto.getVariantId(), warehouseId)
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Không tìm thấy thông tin tồn kho của biến thể ID " + itemDto.getVariantId() + " trong kho "
                                    + warehouseId));

            if (stock.getQuantity() < itemDto.getQuantity()) {
                throw new BadRequestException("Biến thể sản phẩm ID " + itemDto.getVariantId()
